...
bandwidth = .8#about 70*bandwidth miles
...
for k, col in zip(range(n_clusters_), colors):
    ...
    pairs = [(X[my_members, 0], X[my_members, 1]),(cluster_center[0], cluster_center[1])]
    coords = [m(y,x) for x,y in pairs]
    plt.plot(coords[0][0], coords[0][1], col , marker = ',', markersize = .05)
    plt.plot(coords[1][0], coords[1][1], 'o', markeredgecolor='k', markersize=.05)
plt.title('Estimated number of clusters: %d Bandwidth: %.1f degrees of latitude \n Noise Threshold %d' % (accepted_clusters,bandwidth,cluster_limit)\
    ,fontsize = 12)
